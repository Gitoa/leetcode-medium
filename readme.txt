...
46.permutation.作弊方法直接用next_permutation()，但是要注意next_permutation前进行下排序，它只会返回在初始值之后的permutation。个人实现采用了递归，主要的话，注意容器的erase和insert方法。递归其实是DFS的，DFS用标记记录是否加入，我直接在传入时去掉了已加入的数字。此外还有采用swap的方法。
